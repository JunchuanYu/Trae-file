...
### Discussion
<del style="color:red;">This study integrates InSAR deformation phase and multi-source remote sensing data, and employs the MSegFormer model to realize intelligent identification of landslide-related deformation anomaly zones. The results demonstrate that multi-modal data (topography, optical imagery, etc.) are crucial for distinguishing landslide deformations from anthropogenic engineering disturbances, and relying solely on InSAR tends to cause misjudgment. The proposed MSegFormer achieves an F1-score of 80.89%, outperforming DeepLabV3+ and Attention U-Net. Benefiting from the global self-attention mechanism and multi-scale feature extraction capability of the Transformer architecture, the model can effectively capture irregular deformation boundaries and maintain robustness under strong noise conditions. The discrepancy in the number of identified deformations between ascending and descending orbits (71 from descending orbit and 20 from ascending orbit) reflects that the dominant deformation direction in the study area is more consistent with the line-of-sight direction of the descending orbit, verifying the necessity of multi-orbit observations. The model successfully identifies numerous weak deformation signals and previously unrecorded deformation anomaly zones, exhibiting favorable noise suppression and preliminary screening performance. Limitations of this work include subjectivity in sample labeling, spatial scale mismatch among multi-source data, and the lack of consideration for non-linear deformations. Future work will expand the cross-regional sample database and introduce time-series InSAR features to improve the generalization and practicality of the model.</del>

<strong style="color:red;">(1) Importance of multi-source remote sensing for discriminating non-landslide deformation anomaly zones. The identification of two non-landslide-related deformation anomaly zones in descending data—both validated as urban construction sites with flat terrain and funnel-shaped deformation signatures—highlights that InSAR phases alone are insufficient for reliable screening. Incorporating optical imagery and topographic attributes (e.g., elevation and slope) provides critical complementary constraints to suppress false positives and improve the credibility of landslide-related deformation anomaly zone mapping.</strong>
> [中文对照] （2）非滑坡形变区分能力。在降轨数据中，模型识别出2处非滑坡关联形变。经光学核实，均位于城区在建工地，地势平坦，形变呈局部漏斗状，与滑坡的斜坡单元形变模式明显不同，表明MSegFormer能有效利用地形和光学特征区分人为工程活动与滑坡形变。结果表明，多模态数据（地形、光学等）对区分滑坡形变与人为工程扰动至关重要，单靠InSAR易误判。

<strong style="color:red;">(2) Advantages of the MSegFormer framework. First, MSegFormer is compatible with multi-channel inputs, enabling unified feature learning from InSAR deformation phases, optical images, and DEM-derived factors through configurable input channels and lightweight fusion. Second, its hierarchical Transformer encoder with global self-attention, Mix-FFN, and multi-scale feature representation enhances boundary delineation of deformation anomaly zones and improves robustness under strong noise, which is consistent with the observed performance gains over CNN-based baselines.</strong>
> [中文对照] MSegFormer的F1值达80.89%，优于DeepLabV3+和Attention UNet，得益于Transformer的全局注意力与多尺度特征提取能力，能有效捕捉不规则形变边界并在强噪声下保持稳健。模型成功识别了多处微弱信号及以往遗漏的形变区，展现出良好的噪声抑制与初筛能力。

<strong style="color:red;">(3) Necessity of multi-orbit InSAR observations. The large discrepancy between descending (71) and ascending (20) detections indicates strong viewing-geometry dependence in mountainous terrain. Multi-orbit observations can improve completeness and reliability by mitigating line-of-sight sensitivity differences as well as terrain-induced geometric distortions (e.g., layover and shadow), thereby reducing missed detections when only a single orbit is used.</strong>
> [中文对照] 升降轨识别数量差异（降轨71处，升轨20处）反映研究区形变主方向与降轨视线向更为一致，证实多轨道观测的必要性。

<strong style="color:red;">(4) Limitations and future work. Current limitations include the subjectivity of sample labeling, spatial-scale mismatch and co-registration errors among multi-source datasets, and the lack of explicit modeling for non-linear deformation processes. Future work will expand cross-regional sample libraries and integrate time-series InSAR features to enhance generalization, while exploring more objective labeling strategies and uncertainty-aware evaluation to support operational deployment.</strong>
> [中文对照] 局限性包括样本标注主观性、多源数据空间尺度不匹配及未考虑非线性形变。后续将扩展跨区域样本库并引入时序InSAR特征，以提升模型泛化性与实用性。

---

### Conclusion
<del style="color:red;">Taking Wudu District of Longnan City as a typical demonstration area, this study obtains surface deformation phase based on Stacking-InSAR, constructs a 10-band multimodal dataset by fusing multi-source remote sensing data, and adopts the lightweight Transformer model MSegFormer to intelligently identify landslide-related InSAR surface deformation anomaly zones. On the test set of Wudu District, MSegFormer achieves an F1-score of 80.89% and an mIoU of 70.28%, outperforming DeepLabV3+, U-Net and Attention U-Net, which verifies the superiority of the Transformer architecture in multi-source remote sensing data fusion and landslide deformation recognition. The model is applied to identify 20 landslide-related deformations from ascending orbit data and 71 from descending orbit data, and can effectively distinguish landslide deformations from anthropogenic engineering disturbance deformations, demonstrating its practical value in detecting geological disaster hazards in complex mountainous areas. With favorable transferability and operational potential, the proposed method can provide technical support for the regular hazard identification, monitoring, and early warning in landslide-prone regions of the West Qinling Mountains.</del>

<strong style="color:red;">In this study, a multimodal intelligent recognition framework based on Stacking-InSAR and the lightweight MSegFormer model was proposed to detect landslide-related deformation anomaly zones in Wudu District, Longnan City. The main conclusions are as follows:</strong>

<strong style="color:red;">(1) The integration of InSAR deformation phases with multi-source remote sensing data (e.g., optical imagery and topography) into a 10-band dataset significantly enhances the model's ability to distinguish landslide deformations from anthropogenic engineering disturbances, thereby reducing false positives.</strong>

<strong style="color:red;">(2) The MSegFormer model achieves superior performance in multimodal data fusion and dense prediction tasks. With an F1-score of 80.89% and an mIoU of 70.28%, it outperforms benchmark models (DeepLabV3+, U-Net, and Attention U-Net) by effectively suppressing noise and delineating irregular deformation boundaries in complex mountainous terrain.</strong>

<strong style="color:red;">(3) The practical application of the model successfully identified 20 and 71 landslide-related deformation anomaly zones from ascending and descending orbit data, respectively. This discrepancy highlights the necessity of multi-orbit observations to mitigate terrain-induced geometric distortions.</strong>

<strong style="color:red;">Overall, the proposed framework exhibits high transferability and operational potential, providing robust technical support for the rapid screening, regular monitoring, and early warning of geohazards in the landslide-prone West Qinling Mountains.</strong>
> [中文对照] 6 结论
